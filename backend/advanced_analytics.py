...
async def get_war_mode_alerts() -> Dict:
    ...
    funding_rate = random.uniform(-0.0003, 0.0015)
    if abs(funding_rate) > 0.0008:
        alerts.append({"type": "FUNDING_SPIKE", "severity": "HIGH" if abs(funding_rate) > 0.0012 else "MEDIUM", "value": f"{funding_rate * 100:.4f}%", "message": "Extreme funding rate detected", "action": "Consider counter-trading the crowd"})
