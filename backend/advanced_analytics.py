...
async def get_derivatives_data(asset: str = "all") -> Dict:
    ...
    for a in assets:
        ...
        assets_data[a] = {
            ...
            "funding_to_record_pct": round(((rec["funding"] - abs(funding)) / rec["funding"]) * 100, 1) if funding > 0 else 100,
...
async def get_risk_engine() -> Dict:
    ...
    if abs(funding_rate) > 0.05:
        overheat_alerts.append({"type": "FUNDING_SPIKE", "severity": "HIGH", "message": f"Funding rate at {funding_rate * 100:.2f}% - market overheated", "asset": "BTC"})
